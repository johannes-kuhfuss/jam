...
The template keeps SeaBIOS for now while the UEFI/SecureBoot boot path is investigated. It still follows the other Talos Proxmox baseline settings: q35 machine type, VirtIO SCSI controller, raw disk, no memory ballooning, virtio networking, and a socket serial console. The root disk uses `cache=none`, which is the Talos-documented alternative to write-through for clustered environments, and `discard=on` for TRIM support on compatible storage.
